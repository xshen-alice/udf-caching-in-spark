...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    var partitionIterator: Iterator[DiskPartition] = hashedRelation.getIterator()
    ...
    new Iterator[Row] {
      def hasNext() = {
        // IMPLEMENT ME
        if(cachingIterator != null && cachingIterator.hasNext) true
        else if(fetchNextPartition) true
        else false
      }

      def next() = {
        // IMPLEMENT ME
        if(hasNext) cachingIterator.next()
	else null
      }
	  ...
	  var data: Iterator[Row] = diskPartition.getData()
