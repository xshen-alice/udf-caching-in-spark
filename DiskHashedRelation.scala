...
protected [sql] final class GeneralDiskHashedRelation(partitions: Array[DiskPartition])
  ...
  override def closeAllPartitions() = {
    // IMPLEMENT ME
    for(index: Int <- 1 to partitions.size) {
      partitions(index - 1).closePartition()
    }
  }
}
...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    // IMPLEMENT ME
    if (inputClosed) {
      throw new SparkException("Input is already closed!")
    }
    data.add(row)
    val newRowIndex: Int = data.size() - 1
    val newSize: Int = measurePartitionSize()
    if (newSize > blockSize){
      data.remove(newRowIndex)
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      override def next() = {
        // IMPLEMENT ME
        if(hasNext) currentIterator.next()
	else null
        //null
      }
      ...
      override def hasNext() = {
        // IMPLEMENT ME
	//println(currentIterator.hasNext)
        if(currentIterator != null && currentIterator.hasNext) true
        else if(fetchNextChunk()) true
	else false
      }
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    if(input.hasNext) {
      ...
      val data: Array[Row] = input.toArray
      val key: Array[Int] = data.map(x => keyGenerator(x).hashCode % size)

      for (index: Int <- 1 to data.size) {
        if (partitions(key(index - 1)) == null) partitions(key(index - 1)) = new DiskPartition(key(index - 1).toString, blockSize)
        partitions(key(index - 1)).insert(data(index - 1))
      }
      for(i: Int <- 1 to size) {
        partitions(i - 1).closeInput()
      }
      new GeneralDiskHashedRelation(partitions)
    }
    else {
      val partitions: Array[DiskPartition] = new Array[DiskPartition](0)
      new GeneralDiskHashedRelation(partitions)
    }
  }
}
