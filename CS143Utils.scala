  ...
  def apply(
    ...
    { input =>
      new Iterator[Row] {
        ...
        def hasNext() = {
          // IMPLEMENT ME
          val hasNext: Boolean = input.hasNext
          if(!hasNext) cache.clear()
          hasNext
        }
	  ...
	  if(hasNext) {
            ...
            val key: Row = cacheKeyProjection(row)
            var value: Row = cache.get(key)
            if (value == null) {
            value=udfProject(row)
            cache.put(key, value)
            }
            val resultList: JavaArrayList[Any] = new JavaArrayList()
            preUdfProjection(row).iterator.foreach((i: Any) => resultList.add(i))
            value.iterator.foreach((i: Any) => resultList.add(i))
            postUdfProjection(row).iterator.foreach((i: Any) => resultList.add(i))
            Row.fromSeq(resultList.toArray)
	  }
	  else null
        }
      }
